...
app.get('/api/contact-messages', (req, res)=>{
    ...
    connection.connect()
    connection.query(`SELECT message FROM messages`, (err, rows, fields) => {
        if (err) throw err
        rows.forEach((row)=>{
...
app.get('/api/contact-messages/:id', (req, res)=> {
    const id_z_sciezki = req.params.id;
    const connection = mysql.createConnection({
        host: 'localhost',
        user: 'root',
        password: '',
    ...
    connection.connect()
    connection.query(`SELECT message FROM messages where id = ${id_z_sciezki}`, (err, rows, fields)=>{
       res.send(rows[0])
        if (err) throw res.status(404).send('Błąd:(((')
    })
})
...
app.post('/kontakt', function (req, res) {
    ...
    connection.connect()
    connection.query(`INSERT INTO messages (message) VALUES ('${req.body.wiadomosc}') `, (err) => {
        if (err) throw err
